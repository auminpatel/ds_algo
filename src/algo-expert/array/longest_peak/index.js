...
function longestPeak(array) {
  ...
  let longestPeakLength = 0; // The length of the longest peak.
  let i = 1; // The current index in the array.
  ...
  while (i < array.length - 1) {
    ...
    if (!isPeak) {
      // If the current element is not a peak, continue to the next element.
      i++;
      continue;
    }

    // 4. Find the left and right indices of the peak.
    let leftIdx = i - 2;
    ...
    // 5. Calculate the length of the peak.
    const currentPeakLength = rightIdx - leftIdx - 1;

    // 6. Update the longest peak length if necessary.
    longestPeakLength = Math.max(longestPeakLength, currentPeakLength);

    // 7. Move the current index to the end of the peak.
    i = rightIdx;
  }
